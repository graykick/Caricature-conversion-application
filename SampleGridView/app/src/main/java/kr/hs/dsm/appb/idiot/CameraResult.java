...
public class CameraResult extends Activity {
 ...
 /* private String sd = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).getAbsolutePath();
    ...
    private int realEye1CenterX;
    private int realEye2CenterX;
    private int realEye1CenterY;
    private int realEye2CenterY;
        ...
        if (faces.toArray().length >= 1) {
            for (Rect faceRect : faces.toArray()) {
               ...
               /* for (Rect eyeRect1 : eyes.toArray()) {
                    Imgproc.rectangle(ImageMatClone, new Point(eyeRect1.x, eyeRect1.y),
                            new Point(eyeRect1.x + eyeRect1.width , eyeRect1.x + eyeRect1.height),
                            new Scalar(255, 0, 0));
                }*/
                if (eyes.toArray().length >= 2) {
                    ...
                    try {
                        ...
                        Toast.makeText(getApplicationContext(),
                                "ㅅㄱ 오류 realeye1", Toast.LENGTH_SHORT).show();
                    ...
                    //(전체 길이 - 중구(중심구하고 싶은 부위).x) - (전체길이 - 중구(x + width))
                    int rectEye1LengthX = ((faceRect.x + face.width) - realEye1.x) - ((faceRect.x + face.width) - (realEye1.x + realEye1.width));
                    int rectEye2LengthX = ((faceRect.x + face.width) - realEye2.x) - ((faceRect.x + face.width) - (realEye2.x + realEye2.width));
                    realEye1CenterX = rectEye1LengthX/2;
                    realEye2CenterX = rectEye2LengthX/2;

                    int rectEye1LengthY= ((faceRect.y + face.height) - realEye1.y) - ((faceRect.y + face.height) - (realEye1.y + realEye1.height));
                    int rectEye2LengthY = ((faceRect.y + face.height) - realEye2.y) - ((faceRect.y + face.height) - (realEye2.y + realEye2.height));
                    realEye1CenterY = rectEye1LengthY/2;
                    realEye2CenterY = rectEye2LengthY/2;
                    ...
                    Toast.makeText(getApplicationContext(),
                            "눈을 찾지 못했습니다!", Toast.LENGTH_SHORT).show();
                    finish();
                }
...
*/
                        ...
                        Toast.makeText(getApplicationContext(),
                                "iam hard coding", Toast.LENGTH_SHORT).show();
